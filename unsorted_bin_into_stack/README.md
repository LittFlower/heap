...
- 真实 victim 必须能进入 unsorted bin，fake 节点的物理 `size` 与最终 request 必须精确匹配 victim，且 fake next boundary 的 size 检查可读。
- PoC 使用 `malloc(0x410) -> chunksize 0x420`，自然绕开 fastbin；2.26～2.27 若有 tcache，还需让该尺寸不被 tcache 截走或预先填满对应 bin。其他 unsorted 尺寸可用，但链上所有 size 必须同步。
