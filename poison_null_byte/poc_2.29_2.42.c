 ...
 *
 * 2.29 的 backward-consolidation size/prev_size 检查终止旧 stale-prev_size 链。本分支改用 largebin 残留 fd/bk/nextsize 指针：在 prev 内造小 0x10 的 fake chunk，用低两字节改写让 a->bk 与 b->fd 回指 fake chunk，再 off-by-null 清 victim 的 PREV_INUSE，合法 unlink fake chunk并获得重叠。
 *
