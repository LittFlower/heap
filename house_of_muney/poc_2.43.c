 ...
 * glibc 2.43。它先覆盖一个 mmap chunk 的 size，同时保留 IS_MMAPPED 位来
 * 模拟漏洞。核心流程是把这个 mmap chunk 的长度扩大到覆盖相邻映射，free
 * 触发 munmap 之后再重新 mmap，从而拿到一段重叠区域。成功的判据是从新旧
 * 两个指针都能观察到同一块内存。这个 PoC 只展示 mmap overlap 这个原语
 * 本身；如果要完整地"偷到" libc 的映射，还要看实际映射是否相邻。
 *
...
int main()
{
	...
	mmap_chunk_3[-1] = ((0xFFFFFFFFFD & mmap_chunk_3[-1]) +
	                    (0xFFFFFFFFFD & mmap_chunk_2[-1])) | 0x12;
