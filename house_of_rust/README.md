...
原作者公开的仓库只给出了阶段说明，并没有公开完整的 exploit。汇总文件里的五个阶段都是真实执行的，但每个阶段都用独立子进程隔离，避免上一阶段故意破坏的 bin 状态污染下一阶段；这样做只是为了方便逐段验证，不能把它冒充成题目里那种约 65 次分配、共享同一条堆时间线的完整链。真正的端到端 exploit，仍然要按题目实际的 add/edit/free 菜单、slot 生命周期、WAF overlap 和附件 Build ID 来落地。
...
`free_hook` 这个文件只标注了 2.32～2.33，因为从 2.34 起主路径已经不再消费这个 hook 了。这里不打算加入动态查符号、解析版本号之类的兼容代码，版本边界就直接体现在文件名和版本表里。
...
3. largebin 文件用 0x430/0x420 这对物理 chunk 构造插入场景，精确断言写入的值就是新 victim 的 chunk 头地址，而不是随便一个可写地址。

原版 Rust 会在第二阶段和第三阶段各用一组不同的 largebin，第二组只是换了一个 largebin 尺寸，写入的原语本身没有变化。迁移到具体题目时，要把这三个子原语重新拼回同一条共享的堆时间线，而不是各自独立运行。

### stdout 泄漏这个最终触发点

这个文件的输入原语是"已经拿到对 `_IO_2_1_stdout_` 字段的覆盖能力"。程序会直接把 `_IO_write_base/_IO_write_ptr` 指向 `secret`，再调用一次 `fflush`，终端打印出 `stdout 泄漏成功` 就说明这条消费路径走通了。

需要说明的是，它只验证消费端本身，不验证原版 Rust 里那套 1/16 概率的 libc 低位猜测，也不验证 tcache 头是否真的已经被导向 stdout。实战中必须在第一次真实的 stdout 活动处下断点确认；仅仅看到"FILE 字段好像被改了"并不等于已经完成泄漏。

### `__free_hook` 这条边界

glibc 2.34 的关键变化不是兼容符号还能不能解析出来，而是 `free` 的主路径已经不再调用这个 hook 了。所以本目录只保留 2.32～2.33 这个窗口内最简单直接的 PoC：把 `__free_hook` 赋值成 `win`，调用一次 `free(chunk)`，然后断言回调确实被命中。
...
[`poc_summary_2.32.c`](./poc_summary_2.32.c) 依然是可以从上到下线性读下去的：`main` 里依次出现五个阶段，父进程逐段等待子进程执行成功再往下走。前三段真实执行对应的 bin 操作；第四段真实覆盖并消费 stdout；第五段真实写入并消费 `__free_hook`。用子进程隔离只是为了不让上一段故意破坏的伪 bin 状态影响下一段，不是在封装利用步骤。

文件末尾的注释里保留了原先由条件检查器负责的那部分内容：约 65 次分配、0x1b00 的最大 request、两组尺寸类、低四位猜测、stdout 触发点，以及 2.34/2.41 这两个关键版本边界。这些内容明确标注为题目迁移用的伪代码，不会被误当成运行一次就能自动满足的前置条件。

## 从源码看版本边界

原版的完整链可以拆成这几步：

1. 用约 65 次分配，准备好 14 个 0x90 尺寸类、15 个 0xa0 尺寸类、两组 largebin，并制造出 WAF（释放后写）造成的重叠；
2. TSU+ 借助这种双重归属破坏 smallbin 的 `fd/bk`，再用第一次 largebin 写修复 `fd`，从而拿到 tcache 元数据内部的一个 chunk；
...
- 2.33：三个堆管理器组件依然成立，hook 也仍会被消费，只是原作者没有针对这个版本发布完整的迁移；
- 2.34～2.40：TSU+、TSU、largebin 这几个组件以及 stdout 这个最终触发点仍可以分别验证，但原版依赖的 hook 终点已经失效；
- 2.41：smallbin 到 tcache 的 stashing 代码被重构，目前这种形式的 TSU/TSU+ 也就随之失效；
- 2.42：largebin 的 nextsize 检查加上了反向完整性校验，这是比 2.41 更晚的另一道边界。

所以说"House of Rust 在 2.34 上不可用"，这个结论只对原版的完整链成立：前半段的思路并没有因此同时失效，真正失效的是第五阶段的最终触发点；如果换一个终点，还是可以研究怎么迁移，但必须重新给出一套端到端的成功判据。而"2.41 上不可用"是另一回事，是前半段的堆管理器组合本身先断了一环，不是简单改个偏移就能修复回同一条 TSU 链。
