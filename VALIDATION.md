...
- 两个 House of Muney 条目按上游性质标为 ASLR/映射相关，最终轮都成功；Roman 的低字节位置已用每次排布实际地址作教学 oracle，两个端点都是 strict。
- `small_bin_attack` 的 direct 2.23/2.39/2.43 与 tcache-stashing 2.39 汇总入口都纳入正式矩阵；2.43 direct 使用 16-slot tcache 专用链。
- 新增的标准流 IO_FILE 任意读写、freed/in-use chunk overlap 与 sysmalloc old-top free 都在范围两端（glibc 2.23/2.43）运行成功；旧 freed-unsorted overlap 只在它的有效端点 2.23 纳入正向矩阵。
- Large tcache 的 2.42 `>=` 取块、2.43 精确尺寸，以及两版 poisoning 都通过；PoC 明确在启动前提高默认关闭的 `glibc.malloc.tcache_max`。
...
- TSU+ 与 TSU++ 都在 2.27/2.40 通过；相同 PoC 在 2.41 分别不再返回伪 chunk、也不再产生 libc 地址写，按预期断言失败。
...
- House of Apple1 不再误用 `_IO_wstr_overflow` 解释：真实 `_IO_wstrn_overflow` 八字段已知值写在 2.23/2.24/2.36 通过；Obstack 旧后端在 2.23/2.36、Snake 新 printf_buffer 后端在 2.37/2.43 都以真实 `chunkfun(extra_arg,size)` 回调通过。
- House of Apple3 的三段 codecvt ABI 都经真实 `fgetwc→_IO_wfile_underflow` 验证：2.23/2.29 直接 `codecvt+0x18` 回调、精确 2.30 的 `step@codecvt+0x8`、2.31/2.43 的 `step@codecvt+0x0` 全部通过。
- House of Kiwi 的 top-size corruption 在 2.23/2.35 都进入 `__malloc_assert→fflush(stderr)` 回调；同一程序在 2.36 只打印 `__libc_message` fatal 并退出 134，确认触发器末版为 2.35。
...
- House of Pig 的旧 `_IO_strfile` allocate/free 回调在 2.23/2.27 都被真实消费；现代直接扩容最终触发点在 2.28/2.43 逐字节验证 malloc/memcpy/free/memset 效果。
- House of Banana 的真实主程序 link_map 动态项被修改后，2.23/精确 2.30/2.43 都经真实 `exit→_dl_fini→DT_FINI_ARRAY` 调用受控回调；回调用 `_exit(0)`，父进程严格排除普通 `exit(113)` 假阳性。
...
- House of Lore 三个分支都改用 `malloc == fake user address` 严格判据，并在 2.23、2.41、2.42、2.43 通过。House of Roman 的 fastbin 路径在 2.23/2.24、tcache 重排路径在精确 2.26/2.27 都以 `__malloc_hook -> _exit(0)` 通过。
- 原语边界复核纠正了 House of Gods 的过强范围。本目录原版最小链只在 2.23/2.24 严格运行，2.25 沿用相同上游布局。stock 2.26 已有 tcache，且隐藏 `narenas` 相对位置随构建变化，所以归为"尚需独立迁移"而不是"思想失效"。真正的结构硬边界在 2.27：`malloc_state.have_fastchunks` 使原链把 `main_arena+8` 当 size 的条件变成 0/1。
...
- TSU+/TSU++ 在 glibc 2.41 的负测试都退出 134；日志保存在 `build/validation/negative__2.41__tsu_*.log`，确认 `e2436d6` 是行为硬边界。
...
- 7-slot Lore PoC 在 2.43 退出 134，日志为 `build/validation/negative__2.43__house_of_lore_7slot.log`；确认默认 count=16 会让旧排布的 victim 还从 tcache 返回，必须换专用链。
- Apple1 的 2.23～2.36 C PoC 在 2.37 不能产生任何目标字段写，按严格断言退出 134；结合 `118816de3383` 删除函数/jump table，确认 2.36/2.37 是硬边界。日志为 `build/validation/negative__2.37__house_of_apple1.log`。
...
- Pig 旧回调 PoC 在精确 glibc 2.28 中两个回调计数还是零，退出 134，确认 `4e8a6346` 是旧式最终触发点与现代直接 malloc/free 最终触发点的硬分界。
- 经典 Unsorted Bin Attack 与 Unsorted Bin Into Stack 的 2.23～2.27 PoC 在精确 glibc 2.28 都退出 134，fatal 文本为 `corrupted unsorted chunks 3`；确认 `bdc3009` 已在 2.28 加入首道 `bck->fd==victim` 检查，而不是等到 2.29。
...
- House of Storm 在精确 glibc 2.28 中已选出合法 fake size，但第二轮 fake-victim 摘链还是退出 134，fatal 为 `corrupted unsorted chunks 3`；证明不是 size-bit 概率失败。日志为 `build/validation/negative__2.28__house_of_storm.log`。
...
- House of Husk 的 largebin 双表投递链在 2.27/2.35/2.37/2.39/2.41 都真实进入版本标记回调。2.41 旧偏移会把堆地址写到错误的 libc 可写区，还会假退出 0；已用当前 `libc6-dbg` Build ID 更正三个隐藏符号偏移。
- 新增 fastbin→tcache refill PoC 在 glibc 2.43 退出 134，fatal 为 `double free detected in tcache 2`；free 不再落入 fastbin，确认这个子型与其他 Fastbin Dup 一样止于 2.42。

静态审计：

- 141 个 C PoC 都包含逐步中文说明；
...
- 每个手法目录都有 README，README 中枚举的 PoC 文件都存在。

Corrosion、Crust、Rust 是强题目菜单/Build ID 绑定的多原语组合。目录只保留可运行的 C 子原语、汇总演示和 C 文件末尾的迁移伪代码，不把局部阶段成功冒充端到端 RCE。这轮源码复核还纠正了：Corrosion 原文完整链是特定 2.27、Addendum 是特定 2.29；Crust 的超大尺寸 fastbin 指针搬运在 `global_max_fast` 于 2.37 缩为 `uint8_t` 时已经结束，而不是延长到 2.40/2.42。
