...
int main(void) {
	...
	// 2.43 先做一次 free(malloc(0x1e0))，把 tcache_perthread_struct 稳定地推到页内偏移 0x1f0 处，省去对地址低 4 位的爆破。
	free(malloc(0x1e0));
	...
	void *tcache = (void *)((long)mid & ~0xfff) + 0x190;
	void *fill[0x10];
	...
	*(unsigned long *)first = (unsigned long)(page + 0x400);
	*(unsigned long *)(last + 0x8) = (unsigned long)(page + 0x400);
