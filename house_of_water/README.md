...
- 当前 x86-64 布局要求能反复申请 request `0x88`、物理 `chunksize=0x90` 的工作块；这些块在 tcache/unsorted 与 metadata fake chunk 之间建立重叠。guard 的 `0x20` class 和触发 unsorted 排序的大 request 也必须可用。
- 还要把两个工作块临时伪装成另一组精确尺寸来写 tcache entries：2.32～2.41 用物理 `0x330/0x320`，2.42～2.43 用 `0x320/0x310`，随后恢复 `0x90` header。
- `0x90` 与 tcache metadata 中被解释为 `prev_size/size` 的字节位置绑定，不能只把所有 `malloc(0x88)` 换成任意 class。当前确定性排布还分别使用 2.42 的 `malloc(0x4e0)`、2.43 的 `free(malloc(0x1e0))` 固定 metadata 页内位置；换尺寸或 ABI 必须整套重算。
