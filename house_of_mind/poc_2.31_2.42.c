 ...
 * glibc 2.31～2.42。它借助按 HEAP_MAX_SIZE 对齐的堆布局，再单字节改写
...
int main(){
	...
	/*
	 * 2.31~2.42 的 malloc_state 里，fastbinsY 的起始偏移仍是 0x10；0x60
	 * 的 chunk 对应 fastbinsY[4]，数组下标又贡献了 0x20，所以写入位置
	 * 依旧是 fake_arena+0x30。safe-linking 保护的是 fastbin 链表里的 fd
	 * 指针，而这里利用的是"arena 槽直接写入链头"这一步本身，不需要伪造
	 * 被编码过的 fd，所以整体思路仍然成立。
	 */
