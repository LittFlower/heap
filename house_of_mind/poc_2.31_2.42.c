...
int main(){
	...
	// 先填满 0x60 tcache；之后释放 victim 才会越过 tcache，进入需要利用的 fastbin 路径。
	...
	/*
	 * 2.31~2.42 的 malloc_state 中 fastbinsY 起始偏移仍为 0x10；0x60
	 * chunk 对应 fastbinsY[4]，数组下标贡献 0x20，因此写入位置仍是
	 * fake_arena+0x30。safe-linking 保护 fastbin 链表中的 fd，但这里利用
	 * 的是“arena 槽写入链头”本身，不需要伪造被编码的 fd，所以思想仍成立。
	 */

	free(fastbin_chunk); // free 根据伪 ar_ptr 收链，把该堆指针写到 target_loc。

	// 对本版本的 0x60 victim，实际命中偏移为 0x30；断言验证目标得到堆指针。
