...
- 被伪装为 `NON_MAIN_ARENA` 后 free 的 victim 必须属于正常 fastbin 范围，且 fake arena 中被命中的 `fastbinsY[fastbin_index(chunksize)]` 槽必须可读写。
- PoC 用 `malloc(0x50)`，物理 `chunksize=0x60`。可改用其他 fastbin class，但 fake arena 槽偏移、tcache 填充和相邻 size 检查都要跟着 `fastbin_index` 重算。
...
- 2.37 global_max_fast 变成 uint8_t，但正常 fastbin 尺寸还够用。
