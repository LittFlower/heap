 ...
 * glibc 2.23～2.25。它借助按 HEAP_MAX_SIZE 对齐的堆布局，再单字节改写
...
int main(){
	...
	/*
	 * 2.23~2.25 的 malloc_state 里，fastbinsY 的起始偏移是 0x8；0x60 的
	 * chunk 对应 fastbinsY[4]，数组下标又贡献了 0x20，所以实际写入位置
	 * 就是 fake_arena+0x28。构造 fake_arena 的时候，思路跟 unsorted-bin
	 * attack 里用 target-0x10 是一样的：先按目标字段的偏移，反向把结构
	 * 体基址修正到正确的位置。
	 */

	free(fastbin_chunk); // free 会按照伪 ar_ptr 走收链逻辑，把这个堆指针写到 target_loc。

	// 对本版本的 0x60 victim 来说，命中的偏移应该是 0x28；用断言验证目标确实拿到了堆指针。
