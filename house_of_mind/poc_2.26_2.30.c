...
int main(){
	...
	/*
	 * 2.26~2.30 的 malloc_state 中 fastbinsY 起始偏移是 0x10；0x60
	 * chunk 对应 fastbinsY[4]，数组下标再贡献 0x20，所以实际写入位置为
	 * fake_arena+0x30。该结构偏移是版本相关量，必须依据附件 libc 的
	 * malloc_state 定义确认，而不能沿用 2.23 的 0x28。
	 */
