...
- 原语效果：绕过 tcache double-free 检测，再做出可反复利用的 chunk overlap/poisoning。
...
- victim 所在 class 必须能进入 tcache，同时前块在 tcache 满/绕过后必须走可立即合并的普通 free 路径；在仍有 fastbin 的版本中，最省事的是选 `chunksize > get_max_fast()` 的 small-tcache class。
- PoC 用 `malloc(0x100)`，物理 `chunksize=0x110`；这是跨版本稳定示例，不是唯一值。所有用于填满 tcache、prev、victim 和最终 poisoning 的请求必须保持这一 class，2.43 默认还要准备 16 个槽位。
...
- 2.42/2.43 tcache 元数据变化后还有对应 PoC。
