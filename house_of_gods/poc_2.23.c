 ...
 * 手法：house_of_gods
 * 文件标注范围：2.23
 * 模拟漏洞：泄露 heap/libc 且能劫持 main_arena.next、system_mem 与 narenas。
 * 核心流程：把 binmap 当 fake chunk，构造 fake arena，再经 arena 重用逻辑让 thread_arena 指向它。
 * 成功判据：后续 malloc 受 fake arena 控制；2.27 的 arena/fastbin 加固终止本链。
 *
