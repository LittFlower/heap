/*
 * 中文导读：本文件对应 House of Gods 原版最小链，验证范围是 glibc
 * 2.24～2.25（另有独立的 2.23 分支）。模拟的漏洞是对一个 unsorted
 * chunk 做一次 write-after-free，外加一次 heap/libc 地址泄露；核心
 * 流程是把 main_arena.binmap 当作 fake chunk，覆盖 main_arena.next
 * 与 system_mem，再通过 reused_arena 让 thread_arena 指向这个 fake
 * arena。成功判据是后续 malloc 会从 fake arena 的栈上 fastbin 返回
 * 伪造的 fake chunk。
 *
 * 重要边界：原作者写的是“<2.27”，但 stock 2.26 已经引入 tcache，公开
 * 源码不能无条件原样运行，它需要针对 tcache 路由和 Build-ID 的 hidden
 * 偏移做适配。2.27 的 have_fastchunks 又破坏了 main_arena fake chunk
 * 的 size/bk 重叠关系，所以不能只改一个偏移就继续把它称为原版的
 * 单-UAF House of Gods。
 */
