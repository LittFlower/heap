...
**一句话**：保留合法的主 vtable `_IO_wfile_jumps`，利用未被单独校验的 `_wide_data->_wide_vtable`，经 `_IO_wfile_overflow→_IO_wdoallocbuf` 调用受控 `doallocate`。

| 版本 | `_wide_vtable` 偏移 | 备注 |
|---|---|---|
| 2.24～2.29 | `wide+0x130` | 2.24 起使用合法 `_IO_wfile_jumps` |
| 2.30 | `wide+0xf0` | `09e1b0e` 删除 legacy codecvt 函数表，单版本过渡 ABI |
| **2.31～2.43** | `wide+0xe0` | `70c6e15` 缩小 `_IO_iconv_t`；2.34 hooks 删除不影响最终触发点 |

最终触发点本身覆盖 2.24～2.43；2.42/2.43 仍从 fake wide vtable 取 doallocate，但经典 largebin 投递已失效，投递原语需另选。
...
- **最终触发点没有固有 chunk size 要求。** fake FILE、wide data 和 fake wide vtable 只需要有足够的连续可写空间并满足指针对齐，不必来自某个 bin。
- 2.24～2.29 / 2.30 / 2.31+ 的 `0x130 / 0xf0 / 0xe0` 是结构字段偏移，不是 `malloc` request。若用 largebin 等方式投递，再单独满足该投递手法的尺寸条件。
...
源码里还能走到最终触发点，不代表旧利用链还能用。投递方式、私有结构和控制流终点都要按附件 libc/ld 的 Build ID 复核。
...
三份 C 文件直接写出各自 ABI 的 fake FILE、wide data 和 fake wide vtable 字段，不另存布局生成器。2.42～2.43 继续使用 `+0xe0`，但投递原语必须另选。
