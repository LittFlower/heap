...
int main()
{
	...
	// 准备栈上目标。glibc 2.32 的 safe-linking 提交同时增加链节点对齐检查，
	// 因此 target 必须满足 0x10 malloc 对齐；提交参考：
	...
	// 在数组内寻找最低 4 位为零的地址，保证 poisoned next 可被 malloc 接受。
	...
	*(size_t*) &b[real_b_size-sizeof(size_t)] = fake_size;
	...
	// safe-linking 编码需要已知保存 next 的 storage 地址；Einherjar 本身已要求
	// 堆泄漏来计算 fake prev_size，因此直接复用同一泄漏，不增加新原语。
