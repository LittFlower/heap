 ...
 * 成功判据：最终 malloc 返回 target。2.42 重构了大 tcache 和 top 初始化的
 *   逻辑，比早期分支多了一轮截短 top、再借一次大块分配促成 consolidate，
 *   才能把足够的 chunk 挪进 tcache；目标地址也换成了自带合法 size 的对齐
 *   数组，用来绕开这个版本新增的 size 校验。
 *
...
/**
 * 上游作者在 glibc 2.34 和 2.39 上，针对 x86-64、x86、AArch64 三种架构都
 * 验证过这套堆布局。
 *
...
int main() {
  ...
  *top_size_ptr = new_top_size;
  ...
  malloc(0x10000); // 大请求触发 consolidate，把最新 fastbin chunk 归并并整理到 smallbin。
  malloc(SIZE_1); // 从 smallbin 取一块时，把其余同尺寸块按 2.42 规则搬入 tcache。
