 ...
 * 成功判据：最终 malloc 返回 target。2.43 删除了 fastbin，但 top+tcache
 *   这条核心链路依然成立，只是要按新布局多绕一轮截短 top 才能凑够可用的
 *   free chunk；目标地址的处理方式和 2.42 一样，同样要满足新增的 size 校验。
 *
...
int main() {
  ...
  *top_size_ptr = new_top_size;
  ...
  malloc(SIZE_1); // 从 smallbin 取一块，并把其余同尺寸 chunk 搬入 tcache。
