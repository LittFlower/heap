#define _GNU_SOURCE
/*
 * House of Cat 最终触发点的微型 PoC，适用于 glibc 2.31～2.43，x86-64。
 *
...
int main(void)
{
    ...
    /* _IO_wide_data 在 x86-64 上的 _wide_vtable 位于 +0xe0。
       用 qword 数组表示，可以让 CTF 选手直接把数组索引对应到 payload 里的字节偏移。 */
    ...
    /* 2.31 重写 _IO_iconv_t 之后，_wide_vtable 缩到了 +0xe0；这个布局一直保持到 2.43。 */
