#define _GNU_SOURCE
/*
 * House of Cat 最终触发点的微型 PoC，适用于 glibc 2.30，x86-64。
 *
...
int main(void)
{
    ...
    /* 用 qword 数组来表示 _IO_wide_data，方便 CTF 选手直接把数组索引
       对应到 payload 里的字节偏移；2.30 的 _wide_vtable 固定在 +0xf0。 */
    ...
    /* 2.30 删除了旧版 codecvt 函数表，但 _IO_iconv_t 仍是 union，所以 _wide_vtable 位于 +0xf0。 */
    const size_t wide_vtable_offset = 0xf0;
