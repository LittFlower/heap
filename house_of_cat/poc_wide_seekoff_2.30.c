#define _GNU_SOURCE
/*
 * House of Cat 最终触发点微型 PoC：glibc 2.30，x86-64。
 *
...
int main(void)
{
    ...
    /* 用 qword 数组表达 _IO_wide_data，让 CTF 选手直接把索引对应到
       payload offset；2.30 的 _wide_vtable 固定在 +0xf0。 */
    ...
    /* 2.30 删除 legacy codecvt 函数表，但 _IO_iconv_t 仍是 union，_wide_vtable 位于 +0xf0。 */
