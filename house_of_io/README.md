...
- 被伪造/回收到 metadata 的节点必须是一个**启用的 tcache size class**，泄露 key、伪造 `entries[idx]` 和最终请求必须使用同一 `tc_idx`。
- 当前 PoC 选最小类：`malloc(0x10)` 对应物理 `chunksize=0x20`、`tc_idx=0`。理论上换其他 small-tcache class 也可，但 metadata 下溢距离、count/entry 索引和 fake header 偏移都必须重算，因此不能只替换 malloc 常量。
<!-- CHUNK_SIZE_REQUIREMENTS:END -->

## 版本变化

- 2.32 safe-linking 只保护 chunk 内 next，不保护当时的 tcache entries 头，正好形成旁路。
- 2.34 tcache key 改为随机进程值并调整相关初始化/检查；how2heap 的原始链范围止于 2.33。
- 2.42 后出现的是另一种 metadata hijacking，已经不是原始 House of IO 了。
