...
int main(void) {
	...
	// 先触发 tcache_perthread_struct 分配，稳定后续元数据与 value 的相对页布局。
	free(malloc(0x60));

	// 第一步：为两个不同尺寸的 tcache bin 各准备两块链节点。
	
	// 申请两个 0x28 用户块，对应 0x30 tcache；该 bin 保存第一次保护后的值。
	void *a = malloc(0x28);
	void *b = malloc(0x28);
