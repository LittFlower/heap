/*
 * 中文导读：本文件针对 glibc 2.35，用两次现代 largebin attack 分别写入
 * function table 和 arginfo table。
 *
 ...
 * 同是 glibc 2.35，不同发行版的偏移也可能不同，不能直接照抄本文件写死
 * 的值。
 *
 * 阅读顺序：
 ...
 * 作者：Axura。
 * 目标构建：glibc-2.35-0ubuntu3.8 (Ubuntu 22.04)。
 ...
 * 
 * 编译命令：gcc -no-pie -fno-PIE -O0 -g -o house_of_husk_1_glibc-2.35 house_of_husk_1_glibc-2.35.c
 */
...
void backdoor()
{
	static const char marker[] = "[+] HUSK_2.35_CALLBACK\n";
	backdoor_hit++;
